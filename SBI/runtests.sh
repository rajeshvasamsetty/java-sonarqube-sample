...
mvn -U -s settings.xml -Dmaven.package.skip=true -Dmaven.test.skip=true -Dmaven.compile.skip=true deploy sonar:sonar -Dsonar.host.url=http://52.37.254.239:9000 -Dsonar.login=$SONARUSER_TOKEN -Dsonar.scm.provider=svn -Dsonar.login=$GIT_USERNAME -Dsonar.password=$GIT_PASSWORD
